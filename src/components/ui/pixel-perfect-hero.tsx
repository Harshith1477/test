  ...
  return (
    <div className="relative w-full min-h-[100dvh] bg-background flex flex-col justify-between md:justify-center md:gap-6 py-8 md:py-0 px-2 sm:px-6 overflow-hidden select-none isolate">
      <style>{`
        ...
        .tahoe-glass-text {
            color: transparent;
            background: linear-gradient(135deg, rgba(var(--glass-rgb), 1) 0%, rgba(var(--glass-rgb), 0.4) 25%, rgba(var(--glass-rgb), 0.1) 45%, rgba(var(--glass-rgb), 0.9) 55%, rgba(var(--glass-rgb), 0.2) 75%, rgba(var(--glass-rgb), 1) 100%);
            background-size: 200% auto;
            -webkit-background-clip: text;
            background-clip: text;
            -webkit-text-stroke: 1.5px var(--glass-stroke);
            filter: drop-shadow(0 15px 35px rgba(0,0,0,0.15)) drop-shadow(0 5px 10px rgba(0,0,0,0.05));
      ...
      {/* Marquee Strip */}
      <div className="absolute bottom-12 left-0 right-0 w-full overflow-hidden [mask-image:linear-gradient(to_right,transparent,white_15%,white_85%,transparent)]">
        <div className="flex w-max animate-marquee gap-12 items-center">
          {["SaaS Development", "AI Integrations", "Motion Design", 
            "Full Stack", "Graphic Designing", "Video Editing", 
            "SaaS Development", "AI Integrations", "Motion Design", 
            "Full Stack", "Graphic Designing", "Video Editing"].map((item, i) => (
            <span key={i} className="flex items-center gap-12 text-xs uppercase tracking-widest text-foreground/30 font-medium whitespace-nowrap">
              {item}
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 animate-bounce">
        <span className="text-[10px] uppercase tracking-widest text-foreground/20">scroll</span>
        <svg width="12" height="12" viewBox="0 0 12 12" className="text-foreground/20" fill="none">
